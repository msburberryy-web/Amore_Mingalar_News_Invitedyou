...
// Refactor Gemini service to strictly follow latest @google/genai guidelines
import { GoogleGenAI, Type } from "@google/genai";
import { WeddingData, LocalizedString, ScheduleItem } from "../types";

// Generate localized greeting using Gemini 3 Flash
export const generateGreeting = async (data: WeddingData): Promise<LocalizedString> => {
  ...
  try {
    ...
    return JSON.parse(jsonText.trim()) as LocalizedString;
  } catch (error) {
...
// Fetch venue details using Maps grounding (restricted to Gemini 2.5 series)
...
// Generate schedule using Gemini 3 Flash
