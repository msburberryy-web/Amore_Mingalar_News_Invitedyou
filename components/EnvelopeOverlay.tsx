...
const EnvelopeOverlay: React.FC<Props> = ({ data, lang, onOpen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[700px] aspect-[1/1.3] sm:aspect-[1/1.15] md:aspect-[1.5/1] perspective-2000 cursor-pointer overflow-visible">
        <div className={`relative w-full h-full flex transition-all duration-700 bg-transparent ${isOpening ? 'opacity-0 scale-90' : 'opacity-100 scale-100'}`}>
          <WeddingCardTemplate className="w-full h-full">
            <div className="flex flex-col items-center justify-center h-full w-full py-12 md:py-16 gap-12 md:gap-16">
              <div className="flex flex-col items-center w-full px-4 md:px-12 text-center max-w-[95%] overflow-visible">
                <KanoteOrnament className="w-9 md:w-28 opacity-20 mb-7 md:mb-12" />

                <h1 className={`text-wedding-gold text-2xl sm:text-3xl md:text-5xl font-bold mb-7 md:mb-12 drop-shadow-sm whitespace-nowrap leading-tight ${lang === 'my' ? 'font-myanmar tracking-normal' : 'font-serif italic tracking-wide'}`}>
                  {coverTitleText}
                </h1>

                <div className="flex items-center justify-center gap-3 md:gap-8 text-wedding-gold/70 font-serif italic text-xs sm:text-base md:text-3xl w-full px-6 whitespace-nowrap overflow-visible">
                  <span className="shrink-0">{data.groomName.en}</span>
                  <Heart size={16} className="fill-current opacity-50 shrink-0 md:w-8 md:h-8" />
                  <span className="shrink-0">{data.brideName.en}</span>
                </div>

                <KanoteOrnament className="w-9 md:w-28 opacity-20 mt-7 md:mt-12 rotate-180" />
              </div>

              <p className={`text-wedding-gold/50 text-[10px] md:text-base uppercase animate-pulse font-bold tracking-[0.4em] md:tracking-[1.2em] w-full text-center block ${lang === 'my' ? 'font-myanmar' : ''}`}>
